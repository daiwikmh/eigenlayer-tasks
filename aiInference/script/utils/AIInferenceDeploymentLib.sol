...
import {AIInferenceServiceManager} from "../../src/AIInferenceServiceManager.sol";
...
library AIInferenceDeploymentLib {
    using stdJson for *;
    using Strings for *;
    ...
    struct DeploymentData {
        address aiInferenceServiceManager;
    ...
    function deployContracts(
...

        {
            ...
            result.aiInferenceServiceManager = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
            result.stakeRegistry = UpgradeableProxyLib.setUpEmptyProxy(proxyAdmin);
        }
        deployAndUpgradeStakeRegistryImpl(result, core, quorum);
        deployAndUpgradeServiceManagerImpl(result, core, owner);
    ...
    function deployAndUpgradeStakeRegistryImpl(
        ...
        bytes memory upgradeCall = abi.encodeCall(
            ECDSAStakeRegistry.initialize, (deployment.aiInferenceServiceManager, 0, quorum)
        );
    ...
    function deployAndUpgradeServiceManagerImpl(
        ...
        address aiInferenceServiceManager = deployment.aiInferenceServiceManager;
        address aiInferenceServiceManagerImpl = address(
            new AIInferenceServiceManager(
                core.avsDirectory,
        ...
        UpgradeableProxyLib.upgradeAndCall(
            aiInferenceServiceManager, aiInferenceServiceManagerImpl, upgradeCall
        );
    }
    ...
    function readDeploymentJson(
        ...
        require(vm.exists(fileName), "AIInferenceDeployment: Deployment file does not exist");

        string memory json = vm.readFile(fileName);

        DeploymentData memory data;
        data.aiInferenceServiceManager = json.readAddress(".addresses.aiInferenceServiceManager");
    ...
    function writeDeploymentJson(
        ...
        writeDeploymentJson("deployments/ai-inference/", block.chainid, data);
    }
    ...
    function writeDeploymentJson(
        ...
        address proxyAdmin =
            address(UpgradeableProxyLib.getProxyAdmin(data.aiInferenceServiceManager));
    ...
    function readDeploymentConfigValues(
        ...
        require(
            vm.exists(pathToFile), "AIInferenceDeployment: Deployment Config file does not exist"
        );
    ...
    function _generateContractsJson(
        ...
        return string.concat(
            ...
            '","aiInferenceServiceManager":"',
            data.aiInferenceServiceManager.toHexString(),
            '","aiInferenceServiceManagerImpl":"',
            data.aiInferenceServiceManager.getImplementation().toHexString(),
            '","stakeRegistry":"',
